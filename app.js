...
function renderAuthUI() {
  ...
  const uRow = document.getElementById('username-row');
  if (uRow) {
    if (state.user) {
      uRow.style.display = 'flex';
      const uVal = document.getElementById('username-row-value');
      if (uVal) uVal.textContent = state.user.username ? '· @' + state.user.username : '· ' + (state.lang === 'de' ? 'nicht festgelegt' : state.lang === 'es' ? 'no establecido' : 'not set');
    } else {
...
async function saveUsername() {
  ...
  try {
    ...
    if (res.ok && data.success) {
      ...
      if (res.status === 409) msg = state.lang === 'de' ? 'Benutzername vergeben' : state.lang === 'es' ? 'Nombre de usuario en uso' : 'Username taken';
...
async function refreshFriends() {
  try {
    const res = await fetch('/api/friends/list', { credentials: 'same-origin' });
    if (!res.ok) return;
    const data = await res.json();
...
async function openFriendProfile(friendId) {
  ...
  try {
    const res = await fetch('/api/friends/data?friendId=' + friendId, { credentials: 'same-origin' });
    const data = await res.json();
...
async function fetchBuzzes() {
  if (!state.user) return;
  try {
    const res = await fetch('/api/buzzes', { credentials: 'same-origin' });
    if (!res.ok) return;
    const data = await res.json();
...
function setSyncStatus(text, color) {
  ...
  if (st) {
    st.textContent = text;
    st.style.color = color || 'var(--text-mute)';
  }
}
