...
function showTourStep(i) {
  ...
  document.querySelectorAll('#tour-dots .t-dot').forEach((d, idx) => {
    d.classList.toggle('active', idx === i);
  });
  const nextBtn = document.getElementById('tour-next-btn');
  if (nextBtn) {
    if (i === TOUR_TOTAL - 1) {
      nextBtn.textContent = t('get_started') || 'Get started';
    } else {
...
window.addEventListener('popstate', () => {
  ...
  const modal = document.getElementById('currency-modal');
  if (modal && modal.classList.contains('active')) { closeCurrencyModal(); history.pushState({}, ''); return; }
...
function refreshHome() {
  ...
  let greet;
  if (state.lang === 'de') greet = h < 12 ? t('greeting_morning') : h < 17 ? t('greeting_afternoon') : t('greeting_evening');
  else if (state.lang === 'es') greet = h < 12 ? t('greeting_morning') : h < 17 ? t('greeting_afternoon') : t('greeting_evening');
  else greet = h < 12 ? t('greeting_morning') : h < 17 ? t('greeting_afternoon') : t('greeting_evening');
