...
function applyTheme() {
  ...
  if (dk && lt) {
    dk.style.display = state.theme === 'light' ? 'none' : 'block';
    lt.style.display = state.theme === 'light' ? 'block' : 'none';
  }
}
...
function toggleLanguage() {
  ...
  // Rebuild interest chips with translated names
  buildInterestGrid();
  buildInterestModal();
}
  ...
  if (state.user) {
    authCard.innerHTML =
      '<div style="display:flex;align-items:center;gap:14px;">' +
        ...
        '</div>' +
      '</div>' +
      '<button class="btn secondary" style="margin-top:14px;color:var(--bad);" onclick="signOut()">Sign out</button>';
  } else {
    authCard.innerHTML =
      '<div style="text-align:center;padding:8px 0;">' +
        '<div style="font-size:15px;font-weight:700;letter-spacing:-0.02em;margin-bottom:6px;">Sign in to sync</div>' +
        '<div style="font-size:12.5px;color:var(--text-mute);line-height:1.5;margin-bottom:14px;">Your data stays on this device. Sign in to back it up and sync across devices.</div>' +
      '</div>' +
...
function signIn() { window.location.href = '/auth/google'; }
...
function handleAuthQuery() {
  ...
  if (auth) {
    ...
    else if (auth === 'denied') showToast(state.lang === 'de' ? 'Anmeldung abgelehnt' : state.lang === 'es' ? 'Inicio denegado' : 'Sign in denied');
...
function nextTourStep() {
  tourIndex++;
  if (tourIndex >= TOUR_TOTAL) { finishTour(); return; }
...
function renderReadyPersonalized() {
  ...
  const map = {
    'Saving money': { txt: 'Start your first savings goal', go: 'save' },
    'Managing my budget': { txt: 'Try the budget calculator', go: 'tools-budget' },
    'Making better spending decisions': { txt: 'Check a purchase before you buy', go: 'decide' },
    'Paying off debt': { txt: 'See how long until you\'re debt-free', go: 'tools-debt' },
    'Building an emergency fund': { txt: 'Plan your emergency fund', go: 'tools-emergency' },
    'Tracking my financial goals': { txt: 'Set a savings goal', go: 'save' }
  };
  const first = state.interests[0];
  const action = map[first];
  if (!action) { el.innerHTML = ''; return; }
  el.innerHTML = '<div class="info-card" style="text-align:left;"><span class="tag foryou">' + t('for_you') + '</span><h4>' + action.txt + '</h4></div>';
}
...
function openSubScreen(id) {
  ...
  if (id === 'sub-preferences') refreshPrefDisplay();
...
function refreshPrefDisplay() {
  const cd = document.getElementById('pref-currency-display');
  if (cd) cd.textContent = state.currency;
  const idsp = document.getElementById('pref-interest-display');
  if (idsp) idsp.textContent = state.interests.length + ' ' + (state.lang === 'de' ? 'ausgewählt' : state.lang === 'es' ? 'seleccionados' : 'selected');
}
...
function renderThisWeek() {
  ...
  if (weekSpends.length > 0) stats.push('<div style="display:flex;justify-content:space-between;padding:10px 0;"><span style="font-size:13px;color:var(--text-dim);">' + t('spent_today').replace(':','').replace(' today','') + '</span><span style="font-size:14px;font-weight:600;color:var(--text);font-variant-numeric:tabular-nums;">' + fmt(weekSpent) + '</span></div>');
...
const TIPS_DATA = [
  ...
  { cat: 'Saving', match: 'Saving money', h: 'Save windfalls', p: 'Bonus, refund, gift? Send half to savings automatically. You won\'t miss what you never adjusted to.' },
  { cat: 'Saving', match: 'Saving money', h: 'The 1% challenge', p: 'Increase savings by 1% of income each month. After a year, you save 12% more than when you started.' },
  ...
  { cat: 'Habit', match: 'Tracking my financial goals', h: 'Track small spends', p: 'Snacks, transport, small purchases add up. Logging them for 30 days reveals 10-20% of income you didn\'t notice.' },
...
function importData(event) {
  ...
  reader.onload = (e) => {
    try {
      ...
      if (data.lang) { state.lang = data.lang; localStorage.setItem('ss_lang', data.lang); applyTranslations(); applyLangLabel(); }
...
function checkAffordability() {
  ...
  const L = state.lang === 'de' ? {
    ...
    risky: 'Riskant',
    yes: 'Ja, du kannst',
    moreThan: ' ist mehr als dein Kontostand von ',
    short: 'Du fehlen ',
    butAfter: 'Du kannst ',
    buy: ' kaufen, aber nach ',
    wouldHave: ' hättest du nur ',
    left: ' übrig. Dein Puffer ist ',
    giveMonth: '. Gib ihm einen Monat.',
    afterBuying: 'Nach dem Kauf von ',
    forWord: ' für ',
    stillHave: ' hättest du noch ',
    aboveBuffer: ' über deinem monatlichen Puffer von '
  } : state.lang === 'es' ? {
    notYet: 'Aún no',
    risky: 'Riesgoso',
    yes: 'Sí, puedes',
    moreThan: ' es más que tu saldo de ',
    short: 'Te falta ',
    butAfter: 'Puedes comprar ',
    buy: ', pero después de pagar ',
    wouldHave: ' solo te quedarían ',
    left: '. Tu colchón es ',
    giveMonth: '. Dale un mes más.',
    afterBuying: 'Después de comprar ',
    forWord: ' por ',
    stillHave: ' todavía tendrás ',
    aboveBuffer: ' por encima de tu colchón mensual de '
  } : {
    notYet: 'Not yet',
    risky: 'Risky',
    yes: 'Yes, you can',
    moreThan: ' is more than your balance of ',
    short: 'You are short by ',
    butAfter: 'You can buy ',
    buy: ', but after paying ',
    wouldHave: ' you would have only ',
    left: ' left. Your buffer is ',
    giveMonth: '. Give it one more month.',
    afterBuying: 'After buying ',
    forWord: ' for ',
    stillHave: ', you will still have ',
    aboveBuffer: ' above your monthly buffer of '
  };

  if (price > balance) {
    result.className = 'result show no';
    verdictText = L.notYet;
    resultType = 'no';
    explanation.innerHTML = (state.lang === 'en' ? 'Buying <b>' : '') + '<b>' + name + '</b>' + L.moreThan + fmt(balance) + '.<br><br>' + L.short + '<b>' + fmt(price - balance) + '</b>.';
