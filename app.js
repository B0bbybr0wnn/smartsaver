...
function renderToolsHome() {
  // nothing to init; tools are static
}
...
function calcSavings() {
  ...
  if (!target || !monthly) { out.innerHTML = '<p>Enter a target and monthly saving amount.</p>'; return; }
  const remaining = Math.max(0, target - current);
  const months = monthly > 0 ? Math.ceil(remaining / monthly) : 0;
  out.innerHTML = '<p>You need <b>' + fmt(remaining) + '</b> more.<br>At <b>' + fmt(monthly) + '</b>/month, you will reach it in <b>' + months + ' months</b>.</p>';
}
...
function calcBudget() {
  ...
  if (!income) { out.innerHTML = '<p>Enter your monthly income first.</p>'; return; }
  const status = left >= 0 ? 'surplus' : 'overspend';
  out.innerHTML = '<p>Total allocated: <b>' + fmt(total) + '</b><br>Income: <b>' + fmt(income) + '</b><br><b style="color:' + (left >= 0 ? '#22c55e' : '#ef4444') + ';">' + (left >= 0 ? 'Left over: ' : 'Over budget by: ') + fmt(Math.abs(left)) + '</b></p>';
}
...
function calcDebt() {
  ...
  if (!debt || !payment) { out.innerHTML = '<p>Enter the debt amount and monthly payment.</p>'; return; }
  const monthlyRate = (rate / 100) / 12;
  if (payment <= debt * monthlyRate) { out.innerHTML = '<p style="color:#ef4444;">Your payment is too low to ever pay off the debt. Increase it.</p>'; return; }
  ...
  out.innerHTML = '<p>Payoff time: <b>' + timeStr + '</b><br>Total interest: <b>' + fmt(totalInterest.toFixed(2)) + '</b><br>Total paid: <b>' + fmt((debt + totalInterest).toFixed(2)) + '</b></p>';
}
...
function calcDiscount() {
  ...
  const out = document.getElementById('tDisc-out');
  if (!price || !disc) { out.innerHTML = '<p>Enter price and discount percent.</p>'; return; }
  const save = price * (disc / 100);
  const final = price - save;
  out.innerHTML = '<p>You save <b>' + fmt(save.toFixed(2)) + '</b><br>Final price: <b style="color:#22c55e;">' + fmt(final.toFixed(2)) + '</b></p>';
}
...
function calcPercent() {
  ...
  if (!a || !b) { out.innerHTML = '<p>Enter both numbers.</p>'; return; }
  const pct = (a / b) * 100;
  out.innerHTML = '<p><b>' + a + '</b> is <b>' + pct.toFixed(1) + '%</b> of <b>' + b + '</b>.</p>';
}

// Tool: Emergency Fund
function calcEmergency() {
  const essentials = parseFloat(document.getElementById('te-ess').value) || 0;
  const out = document.getElementById('te-out');
  if (!essentials) { out.innerHTML = '<p>Enter your monthly essentials.</p>'; return; }
  out.innerHTML = '<p>Planning targets:</p><p>1 month: <b>' + fmt(essentials) + '</b><br>3 months: <b>' + fmt(essentials * 3) + '</b><br>6 months: <b>' + fmt(essentials * 6) + '</b><br>12 months: <b>' + fmt(essentials * 12) + '</b></p><p style="font-size:12px;color:#94a3b8;margin-top:10px;">These are planning calculations, not personalized financial advice.</p>';
}
...
function calcTip() {
  ...
  if (!bill) { out.innerHTML = '<p>Enter the bill amount.</p>'; return; }
  const tipAmt = bill * (tip / 100);
  const total = bill + tipAmt;
  out.innerHTML = '<p>Tip: <b>' + fmt(tipAmt.toFixed(2)) + '</b><br>Total: <b>' + fmt(total.toFixed(2)) + '</b></p>';
}

// Tool: Currency Converter (basic, uses fixed sample rates - note this)
function calcConvert() {
  ...
  if (!amt || !rate) { out.innerHTML = '<p>Enter the amount and exchange rate.</p>'; return; }
  out.innerHTML = '<p>Result: <b>' + (amt * rate).toLocaleString(undefined, { maximumFractionDigits: 2 }) + '</b></p><p style="font-size:12px;color:#94a3b8;margin-top:10px;">Enter the current exchange rate manually for accuracy.</p>';
}
