...
async function signOut() {
  // Wipe all local data so the next account starts fresh
  const keysToWipe = [
    'ss_goals', 'ss_decisions', 'ss_contributions', 'ss_spends',
    'ss_snapshot', 'ss_templates', 'ss_streak', 'ss_friends'
  ];
  ...
  state.snapshot = null;
  state.templates = [];
  state.streak = { current: 0, longest: 0, lastDay: null, history: [] };
  state.friends = [];
  state.incoming = [];
  state.outgoing = [];
  state.buzzes = [];
...
async function cloudPullThenMaybePush() {
  ...
  try {
    ...
    const localEmpty = state.goals.length === 0 && state.decisions.length === 0 && state.spends.length === 0;
    if (cloudEmpty && !localEmpty) {
      ...
      if (data.streak && typeof data.streak === 'object') {
        state.streak = {
          ...
          lastDay: data.streak.lastDay || null,
          history: Array.isArray(data.streak.history) ? data.streak.history : []
        };
        saveStreak();
      }
...
function resetApp() {
  ...
  const keysToWipe = [
    'ss_goals', 'ss_decisions', 'ss_contributions', 'ss_spends',
    'ss_snapshot', 'ss_templates', 'ss_streak', 'ss_interests'
  ];
  keysToWipe.forEach(k => localStorage.removeItem(k));
  state.goals = [];
  state.decisions = [];
  ...
  renderHistory();
  renderDataStats();
  showToast(state.lang === 'de' ? 'Daten gelöscht' : state.lang === 'es' ? 'Datos borrados' : 'Data wiped');
}
